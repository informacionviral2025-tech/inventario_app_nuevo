...
class _SalidasInventarioScreenState extends State<SalidasInventarioScreen> {
  ...
  void _procesarSalidas() async {
    ...
    showDialog(
      context: context,
      builder: (context) => AlertDialog(
        title: const Text('Confirmar salidas'),
        content: Text('¿Deseas procesar ${_carritoSalidas.length} salida(s) de inventario?'),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(context),
            child: const Text('Cancelar'),
  ...
  Future<void> _ejecutarSalidas() async {
    try {
      for (var item in _carritoSalidas) {
        ...
        // Registrar la salida
        final success = await _salidasService.registrarSalida(
        ...
        if (success) {
          ...
          await provider.updateStock(articulo.id!, nuevoStock, widget.empresaId);
        } else {
          throw Exception('Error al procesar salida de ${articulo.nombre}');
        }
      }
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Salidas procesadas correctamente'),
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Error al procesar salidas: $e'),
            ...
            children: [
              ...
              SizedBox(
                ...
                child: ElevatedButton(
                  ...
                  child: const Text('Procesar Salidas'),
                ),
              ),
            ],
          ),
